// function to extracts only relevant info and if no parameter is sent, sort ascendingly
  ...
  for (let id in dataSource) {
    ...
    const objectTemplate = {
      ...
      numberOfMissedVotes: round(
        (dataSource[id]["total_votes"] * dataSource[id].votes_with_party_pct) /
          100,
        0
      ),
      percentVotedWithParty: round(dataSource[id].votes_with_party_pct, 1) + "%"
    };
